...
$sql= mysqli_query($conn, "SELECT * FROM productos WHERE descripcion LIKE '%$Texto%' OR nombre LIKE '%$Texto%'");
...
$Categoria=mysqli_fetch_array(mysqli_query($conn, "SELECT categoria FROM productos WHERE descripcion LIKE '%$Texto%' OR nombre LIKE '%$Texto%' LIMIT 1"));
?>
<div class="grey lighten-2" id="datos">
	...
	<div class="hide-on-small-only	col m3 l3">
	  <div class="row">
		...
		<div class="col m9 l9">
		 <br><br> 
		<h5>Categoria: <br><?php echo $Categoria['categoria']; ?></h5>

		<br><b>Filtrar por:</b><br><br>
		<a href="categoria.php?cat=<?php echo $Categoria['categoria']; ?>&f=Precio" class="black-text">Precio <i class="tiny material-icons rigth">attach_money</i></a><br><a href="categoria.php?cat=<?php echo $Categoria['categoria']; ?>&f=New" class="black-text">
	...
	<div class="col s12 m8 l8"><br>
	  ...
	  for ($i=0; $i <=$Filas_pagina; $i++) {
		$inicia=$i*3; 
		$productos = mysqli_query($conn, "SELECT * FROM productos WHERE descripcion LIKE '%$Texto%' OR nombre LIKE '%$Texto%' LIMIT $inicia,3");
	  ?>
		<div class="row">
		<?php
